...
describe("createToken", () => {
  const trip = { id: 1 };
  const leader = { id: 10, is_leader: true };
  const nonLeader = { id: 11, is_leader: false };
  const tokenRow = { id: 5, token: "abc-uuid", label: "ลิงก์แชร์", expires_at: null, privacy_mode: "full", created_at: new Date() };
  ...
  test("rejects non-leader", async () => {
    const result = await createToken(trip, nonLeader);
    expect(result).toMatchObject({ ok: false, error: "leader only" });
  });

  test("rejects when at max token limit", async () => {
    db.one.mockResolvedValueOnce({ n: MAX_TOKENS_PER_TRIP });
    const result = await createToken(trip, leader);
    expect(result.ok).toBe(false);
    expect(result.error).toContain("เกินจำนวน");
  });

  test("rejects invalid label (too long)", async () => {
    db.one.mockResolvedValueOnce({ n: 0 });
    const result = await createToken(trip, leader, { label: "x".repeat(31) });
    expect(result).toMatchObject({ ok: false, error: expect.stringContaining("ชื่อลิงก์") });
  });

  test("rejects invalid privacy_mode", async () => {
    db.one.mockResolvedValueOnce({ n: 0 });
    const result = await createToken(trip, leader, { privacy_mode: "unknown" });
    expect(result).toMatchObject({ ok: false, error: expect.stringContaining("privacy_mode") });
  });

  test("rejects out-of-range expires_in_hours (0)", async () => {
    db.one.mockResolvedValueOnce({ n: 0 });
    const result = await createToken(trip, leader, { expires_in_hours: 0 });
    expect(result).toMatchObject({ ok: false, error: expect.stringContaining("expires_in_hours") });
  });

  test("creates token successfully with defaults", async () => {
    db.one.mockResolvedValueOnce({ n: 0 }).mockResolvedValueOnce(tokenRow);
    const result = await createToken(trip, leader, {});
    expect(result).toMatchObject({ ok: true });
    expect(result.token).toBeDefined();
  });

  test("creates token with explicit expires_in_hours", async () => {
    const rowWithExpiry = { ...tokenRow, expires_at: new Date(Date.now() + 86400_000) };
    db.one.mockResolvedValueOnce({ n: 0 }).mockResolvedValueOnce(rowWithExpiry);
    const result = await createToken(trip, leader, { expires_in_hours: 24 });
    expect(result.ok).toBe(true);
    expect(result.token.expires_at).toBeDefined();
  });

  test("uses default label when label is empty string", async () => {
    db.one.mockResolvedValueOnce({ n: 0 }).mockResolvedValueOnce(tokenRow);
    const result = await createToken(trip, leader, { label: "" });
    expect(result.ok).toBe(true);
  });
});
...
describe("listTokens", () => {
  ...
  test("returns rows from DB", async () => {
    const rows = [{ id: 1, token: "uuid-1", label: "ลิงก์แชร์", view_count: 5 }];
    db.many.mockResolvedValueOnce(rows);
    const result = await listTokens(42);
    expect(result).toEqual(rows);
  });

  test("returns empty array when no tokens", async () => {
    db.many.mockResolvedValueOnce([]);
    const result = await listTokens(99);
...
describe("revokeToken", () => {
  ...
  test("rejects non-leader", async () => {
    const result = await revokeToken(1, 5, nonLeader);
    expect(result).toMatchObject({ ok: false, error: "leader only" });
  });

  test("returns error when token not found", async () => {
    db.one.mockResolvedValueOnce(null);
    const result = await revokeToken(1, 99, leader);
    expect(result).toMatchObject({ ok: false, error: "token not found" });
  });

  test("returns error when token already revoked", async () => {
    db.one.mockResolvedValueOnce({ id: 5, revoked_at: new Date() });
    const result = await revokeToken(1, 5, leader);
    expect(result).toMatchObject({ ok: false, error: "token revoked already" });
  });

  test("revokes token successfully and calls UPDATE", async () => {
    db.one.mockResolvedValueOnce({ id: 5, revoked_at: null });
    const result = await revokeToken(1, 5, leader);
    expect(result).toMatchObject({ ok: true });
    const updateCall = db.query.mock.calls.find(([sql]) => sql.includes("revoked_at = now()"));
...
describe("recordView", () => {
  ...
  test("executes UPDATE with correct tokenId", async () => {
    await recordView(42);
